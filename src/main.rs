...
// Simulation control
const ENABLE_RPC_SIMULATION: bool = true; // ✅ فعال شده با replaceRecentBlockhash!
...
struct GlobalStats {
    ...
    skipped_target_confirmed: AtomicUsize,  // ✅ جدید!
    skipped_simulation_failed: AtomicUsize,  // ✅ جدید!
}
...
async fn unified_worker_thread(
    ...
    for tx_info in rx.iter() {
        ...
        // ═══════════════════════════════════════════════════════════
        // ✅ FILTER 4: چک کردن وضعیت تراکنش هدف - آیا قبلاً confirm شده؟
        // ═══════════════════════════════════════════════════════════
        match jito_client.check_target_transaction_status(&tx_info.signature).await {
            Ok(TargetTxStatus::AlreadyConfirmed) => {
                warn!("⚠️  [W{}] Target tx already confirmed - TOO LATE!", worker_id);
                stats.skipped_target_confirmed.fetch_add(1, Ordering::Relaxed);
                continue;
            }
            Ok(TargetTxStatus::InMempool) => {
                debug!("✅ [W{}] Target tx in mempool - READY!", worker_id);
            }
            Ok(TargetTxStatus::NotFound) => {
                debug!("ℹ️  [W{}] Target tx not found yet - proceeding anyway", worker_id);
            }
            Ok(TargetTxStatus::Unknown) | Err(_) => {
                debug!("⚠️  [W{}] Could not check target tx status - proceeding anyway", worker_id);
            }
        }
        ...
        if simulation.is_profitable {
            ...
            // ✅ دریافت creator_vault مستقیماً از تراکنش قربانی
            ...
            debug!("💰 [W{}] Priority Fees:", worker_id);
            debug!("   Victim: {} μLamp", tx_info.priority_fee);
            debug!("   Front-run: {} μLamp", front_run_priority_fee);
            debug!("   Back-run: {} μLamp", back_run_priority_fee);
            ...
            // ═══════════════════════════════════════════════════════════
            // 🔬 RPC SIMULATION با replaceRecentBlockhash
            // ═══════════════════════════════════════════════════════════
            if ENABLE_RPC_SIMULATION {
                info!("🔬 [W{}] Simulating transactions with RPC (replaceRecentBlockhash=true)...", worker_id);
...
async fn main() -> Result<()> {
    ...
    info!("🌐 Endpoints:");
    info!("   Shreds: {}", shred_endpoint);
    info!("   Geyser: {}", grpc_endpoint);
    info!("   RPC (ERPC): {}", rpc_endpoint);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let skip_target_confirmed = stats_report.skipped_target_confirmed.load(Ordering::Relaxed);
            let skip_simulation_failed = stats_report.skipped_simulation_failed.load(Ordering::Relaxed);
            let total_skipped = skip_no_pool + skip_low_sol + skip_same_block + skip_no_creator + skip_target_confirmed + skip_simulation_failed;
            ...
            info!("║    Target confirmed: {} ({:.1}%)", skip_target_confirmed,
                  if total_skipped > 0 { (skip_target_confirmed as f64 / total_skipped as f64) * 100.0 } else { 0.0 });
            info!("║    Simulation failed: {} ({:.1}%)", skip_simulation_failed,
                  if total_skipped > 0 { (skip_simulation_failed as f64 / total_skipped as f64) * 100.0 } else { 0.0 });
            info!("╠═══════════════════════════════════════════════════════════╣");
            info!("║ 💰 Simulations:");
