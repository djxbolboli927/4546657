...
const SANDWICH_SAFETY_MARGIN: f64 = 0.98;
...
// ✅ CRITICAL FIX: اضافه کردن token_program_id و user_token_account
#[derive(Debug, Clone)]
struct TransactionInfo {
    ...
    token_program_id: String,        // ✅ NEW! Account #8
    user_token_account: String,      // ✅ NEW! Account #5
}
...
// ═══════════════════════════════════════════════════════════════
// ✅ WORKER THREAD - با استفاده از Token Program ID استخراج شده
// ═══════════════════════════════════════════════════════════════
...
async fn unified_worker_thread(
    ...
    for tx_info in rx.iter() {
        ...
        if simulation.is_profitable {
            ...
            // ═══════════════════════════════════════════════════════════
            // 🚀 EXECUTION - با استفاده از Token Program ID استخراج شده
            // ═══════════════════════════════════════════════════════════
            ...
            // ✅ دریافت Creator Vault از تراکنش قربانی
            ...
            // ✅✅✅ CRITICAL FIX: استفاده از Token Program ID استخراج شده!
            let token_program_id = match Pubkey::from_str(&tx_info.token_program_id) {
                Ok(tpid) => tpid,
                Err(e) => {
                    error!("❌ [W{}] Invalid token program id: {}", worker_id, e);
                    continue;
                }
            };

            info!("✅ [W{}] Using Token Program from victim tx: {}", worker_id, token_program_id);
            ...
            // ✅ Build transactions با token_program_id صحیح!
            let front_tx = match tx_builder.build_front_run_transaction(
                &wallet_manager.front_runner,
                &mint,
                &bonding_curve,
                &creator_vault,
                &token_program_id,  // ✅ استفاده از token_program_id استخراج شده!
...
// ✅✅✅ CRITICAL FIX: استخراج token_program_id و user_token_account از تراکنش قربانی
            ...
            if program_id == pump_fun_program_id && instruction.data.starts_with(buy_discriminator) {
                if let Ok(args) = BuyInstructionArgs::try_from_slice(&instruction.data) {
                    if args.max_sol_cost >= MIN_SOL_COST && args.max_sol_cost <= MAX_SOL_COST {
                        ...
                        // ✅ Account #5: user_token_account
                        let user_token_account = instruction.accounts.get(5)
                            .and_then(|&idx| account_keys.get(idx as usize))
                            .map(|pk| pk.to_string());

                        // ✅ Account #8: TOKEN_PROGRAM_ID (Token or Token2022!)
                        let token_program_id = instruction.accounts.get(8)
                            .and_then(|&idx| account_keys.get(idx as usize))
                            .map(|pk| pk.to_string());

                        // ✅ Account #9: creator_vault
                        ...
                        if let (Some(buyer), Some(mint), Some(uta), Some(tpid)) =
                            (buyer_pubkey, mint_pubkey, user_token_account, token_program_id) {
                            ...
                            return Some(TransactionInfo {
                                ...
                                slot: current_slot,
                                creator_vault,
                                token_program_id: tpid,         // ✅ استخراج شده!
                                user_token_account: uta,        // ✅ استخراج شده!
                            });
                        }
                    }
                }
...
async fn main() -> Result<()> {
    ...
    info!("═══════════════════════════════════════════════════════════");
    info!("MEV Bot v16.0 - TOKEN2022 FIX 🚀");
    info!("Workers: {}", WORKER_COUNT);
    info!("RPC Simulation: {}", if ENABLE_RPC_SIMULATION { "ENABLED" } else { "DISABLED" });
    info!("═══════════════════════════════════════════════════════════");
    info!("🎯 CRITICAL FIX:");
    info!("   ✅ Token Program ID: FROM VICTIM TX!");
    info!("   ✅ User Token Account: FROM VICTIM TX!");
    info!("   ✅ Supports Token2022 & Token Program!");
    info!("   ✅ No more IncorrectProgramId errors!");
    info!("═══════════════════════════════════════════════════════════");
